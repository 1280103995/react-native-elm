      ...
      <View style={styles.cartView}>
        ...
        <Row verticalCenter style={{height: px2dp(90), justifyContent: 'space-between',backgroundColor: Color.gray3}}>
          <Column style={{height:px2dp(90),marginLeft:px2dp(160),justifyContent:'center'}}>
            <Text white text={`￥${CartStore.totalPrice}`}/>
        ...
        <Animated.View style={[styles.iconWrap, {
          ...
          {/*购物车图标*/}
          <View style={[styles.iconView, CartStore.totalCount > 0 ? {backgroundColor: Color.theme} : null]}>
            <Image  source={Images.Shop.cart} ref={(cart)=>this.props.cartElement(cart)}
              style={{...wh(50), tintColor: CartStore.totalCount > 0 ? Color.white : Color.gray3}}/>
          </View>
          {/*数量*/}
          {CartStore.totalCount > 0 ?
            <View style={styles.count}>
              <Text white text={CartStore.totalCount} style={{fontSize: px2sp(18)}}/>
            </View> : null}
        </Animated.View>
      </View>
    )
